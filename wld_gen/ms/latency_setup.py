...
for i in range(len(pods)):
    for j in range(len(commands)):
        comm="kubectl -n media-microsvc exec {0} -it -- bash -c \" {1} \"".format(pods[i],commands[j])
        print(comm)
        os.system(comm)
    if svcs[i]==frontend: #U2E latency
        os.system("kubectl -n media-microsvc exec  {0} -it -- bash -c \" tc filter add dev eth0 protocol ip parent 1: prio 1 u32 match ip src 192.168.235.192 flowid 1:13 \" ".format(pods[i]))
        os.system("kubectl -n media-microsvc exec  {0} -it  -- bash -c \" tc filter add dev eth0 protocol ip parent 1: prio 1 u32 match ip dst 192.168.235.192 flowid 1:13 \" ".format(pods[i]))
...
for i in range(len(cloud_pods)):
    for j in range(len(svcs)):
        ...
        if(pods[j] not in cloud_pods):  #check j is not in the CLOUD
            ...
            #Add latency to FROM:
            comm="kubectl -n media-microsvc exec "+  cloud_pods[i]+"  -it --  tc filter add dev eth0 protocol ip parent 1: prio 1 u32 match ip src " +svc_ip+" flowid 1:11 "
            print(comm)
            os.system(comm)
            comm="kubectl -n media-microsvc exec "+  cloud_pods[i]+"  -it --  tc filter add dev eth0 protocol ip parent 1: prio 1 u32 match ip dst " +svc_ip+" flowid 1:11 "
            os.system(comm)
            #Add latency to TO:
