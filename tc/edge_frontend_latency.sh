...
## end of configuration
 ...
 start () {
  ## outbound.
  echo "add latency between edge router and front-end with amount 290ms"
  ...
  tc qdisc add dev $INTERFACE parent 1:11 handle 11:0 netem delay 290ms #5ms 25% distribution normal
  ...
  tc filter add dev $INTERFACE protocol ip parent 1:0 prio 1 u32 match ip dst $FRONTEND_IP flowid 1:11
 ...
 stop () {
  ...
  echo "stop network rule in edge"
  # rm -f $LOCKFILE
}
