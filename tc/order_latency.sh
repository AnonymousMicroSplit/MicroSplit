...
ORDERDB_IP=172.18.0.13
PAYMENT_IP=172.18.0.4
ORDERDBSLAVE_IP=172.18.0.14
CARTS_IP=172.18.0.8
INTERFACE=eth0
## end of configuration
 ...
 start () {
  ## outbound.
  echo "configure order latency to orderdb shipping payment delay 280ms"
  ...
  tc filter add dev $INTERFACE protocol ip parent 1:0 prio 1 u32 match ip dst $ORDERDB_IP flowid 1:11
  #tc filter add dev $INTERFACE protocol ip parent 1:0 prio 1 u32 match ip dst $ORDERDBSLAVE_IP flowid 1:11
  #tc filter add dev $INTERFACE protocol ip parent 1:0 prio 1 u32 match ip dst $SHIPPING_IP flowid 1:11
  #tc filter add dev $INTERFACE protocol ip parent 1:0 prio 1 u32 match ip dst $PAYMENT_IP flowid 1:11
 ...
 stop () {
  ...
  echo "stop network rule in order"
