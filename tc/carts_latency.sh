...
## configuration. 
ORDER_IP=172.18.0.5
CATALOGUE_IP=172.18.0.4
USER_IP=172.18.0.10
SHIPPING_IP=172.18.0.3
QUEUE_IP=172.18.0.9
RABBIT_IP=172.18.0.12
PAYMENT_IP=172.18.0.2
CARTS_IP=172.18.0.8
FRONTEND_IP=172.18.0.7
CATALOGUEDB_IP=172.18.0.11

CARTDB_IP=172.18.0.16
CARTDBSLAVE_IP=172.18.017
...
## end of configuration
 ...
 start () {
  ## outbound.
  echo "configure carts latency to cartdb delay 30ms"
  ...
  # network emulation - add latency. 
   tc qdisc add dev $INTERFACE parent 1:11 handle 11:0 netem delay 30ms #5ms 25% distribution normal
  ...
  tc filter add dev $INTERFACE protocol ip parent 1:0 prio 1 u32 match ip dst $CARTDB_IP flowid 1:11
 ...
 stop () {
  ...
  echo "stop network rule in carts"
